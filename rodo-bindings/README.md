...
Bindings Python para controlar el sistema Chacras vía Modbus RTU.
Funciona tanto contra la blue pill física como contra el simulador por software.
...
```
chacras-softsim/
├── chacras.c           # Código fuente del cliente C
├── chacras_slave.c     # Código fuente del esclavo simulador
├── chacras.py          # Bindings Python (este archivo)
├── start_sim.sh        # Script que levanta el simulador
├── master              # Binario del cliente C (compilado)
...
```python
from chacras import Chacras

with Chacras(port='/temp/ttyV0') as c:

    # Leer estado de batería y armado
    status = c.get_status()
    # {'ibat': 5.0, 'vbat': 24.0, 'armado': 1}

    # Leer estado de un motor (0 a 3)
    motor = c.get_motor(0)
    # {'setpoint': 100, 'sentido': 1, 'velocidad': 100.0, 'corriente': 5.0}

    # Setear setpoint y sentido de un motor
...
Si necesitás mantener la conexión abierta (por ejemplo en un nodo ROS):

```python
c = Chacras(port='/dev/ttyV0')
c.connect()

# ... usarlo en callbacks, timers, etc.

c.disconnect()
```

El puerto por defecto es `/dev/ttyV0`. Para usar con la placa física:

```python
c = Chacras(port='/dev/ttyUSB0')
```
...
./master -p /tmp/ttyV0 -s

# Ver estado de un motor (0 a 3)
./master -p /tmp/ttyV0 -m 0

# Establecer setpoint y sentido de un motor
#   ./master -p /dev/ttyV0 -m <motor> <setpoint> <sentido>
./master -p /tmp/ttyV0 -m 0 100 1

# Armar el sistema
./master -p /tmp/ttyV0 -a 1

# Desarmar
./master -p /tmp/ttyV0 -a 0
```
